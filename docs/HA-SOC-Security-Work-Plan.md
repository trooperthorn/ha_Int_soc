...
Settled by the two live D-21 runs (2026-08-30, output pasted by the owner; the full fact list is recorded in `ha_soc_probe/DOCS.md`): Supervisor 2026.08.0 (GHSA-gh5m-4m97-c95h patched), Core 2026.8.3, Home Assistant OS 18.1, boot slot B, 18.2 offered but not installed; the add-on installs, scans, and reports on the real Supervisor, and the 502 hold-and-retry path recovered from a live Core restart; the observed rating is 1 exactly as documented; the live `get_addons_info()` payload carries every key the sprint 7 inventory needs; both services are registered through the proxy; the pre-fix build's file modes confirmed the DATA-1 finding on disk; backup encryption is on for the default and the one configured agent. Container level: both the add-on and the host use the nf_tables backend (no legacy xtables loaded), so no backend switch precedes item 2.4; `ip6tables` works and the host's LAN and VLAN interfaces carry global IPv6, so item 2.4 is fully unblocked; the container is unprivileged with exactly CapAdd NET_ADMIN under the `docker-default` AppArmor profile; `/var/run` is a symlink to `/run`; with Protection Mode on the Docker socket is absent at both paths, observed live; mounts are minimal and the config directory is not mapped; the live `HA_SOC_RULES` chain exists with its single position-1 `INPUT` jump. One finding: the pairing secret file sat at 0644 inside `/data`; both run scripts now create it 0600 and tighten an existing file at startup.

Still open:

- Whether the Supervisor runs an add-on-provided uninstall hook usable to remove the `INPUT` jump and the chain. The Supervisor has an `app_uninstall` step but no documented add-on-authored pre-uninstall script; treat removal as best-effort and document that an empty chain and one jump may remain (item 2.3, documented).
- UniFi field names per firmware (the `# VERIFY` markers in `unifi.py`); confirmed only against the owner's console.
- The absence of listening sockets opened by the add-on itself is established by script review only: under `host_network` a socket listing inside the container shows the host's ports and cannot isolate the add-on's own (method limit, recorded in DOCS.md).
