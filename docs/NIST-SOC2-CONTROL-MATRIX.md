...
| Device SSH collection: owner-only, allowlisted reads, pinned host keys, output never persisted (ssh_devices.py) | AC-6, CM-8, IA-5, SC-8, SC-23 | CC6.1, CC6.6 | Key-based authentication with a controller-distributed key; no password stored; trust-on-first-use host-key pinning with hard refusal on change |
| UniFi configuration baseline and drift detection (config_ledger.py) | CM-2, CM-6, CM-8 | CC7.1, CC8.1 | Owner-accepted baseline, per-section drift, six-hourly observation audited as `unifi_config_drift`; read-only against the controller, and a partial fetch is refused rather than compared |
