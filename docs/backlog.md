...
- 2026-09-09 ssh_devices.COMMANDS: four entries are marked `verified=False` (`board_info`, `system_cfg`, `mgmt_cfg`, `mca_info`). Their output has not been seen on this estate. Run them from the Device SSH card against a UDB, the Pro Max switch, the Flex and a U7 Lite, then write parsers and flip the flags.
