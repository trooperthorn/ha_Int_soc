...
- 2026-09-09 ssh_devices.COMMANDS: seven entries are marked `verified=False` (`board_info`, `system_cfg`, `mgmt_cfg`, `mca_info`, and the association set `wstalist`, `mca_dump`, `syslog_tail`). Their output has not been seen on this estate. Run them from the Device SSH card against a UDB, the Pro Max switch, the Flex and a U7 Lite, then write parsers and flip the flags. For the association set the parser worth writing is the one that turns hostapd's association, authentication and deauthentication lines into per-client attempts with reason codes, which is the one thing the Wi-Fi join view cannot get from any API; until then the output is read by eye.
