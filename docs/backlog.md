...
- 2026-09-22 Off-box journal shipping (crash-forensics report section 3(a)): the only capture that survives a kernel panic or a host that never comes back at all, since a panic leaves nothing this integration (or anything else running IN the failed boot) can read afterward. Ship the host journal continuously to a box that is not the one that might go down — a small poller against `/api/hassio/host/logs/follow`, or the `addon-promtail`/`ha-addon-syslog` app pattern (Promtail covers host, Supervisor, Core and every app, and pairs with `ha_app_grafana`, already in this fleet). `syslog_export.py`'s RFC 5424/CEF framing, octet counting and bounded queue are the right transport shape to extend with a second source; not started.
- 2026-09-22 ha_app_kiosk Chromium SIGILL: `traps: chromium ... trap invalid opcode` at a constant code offset, 19 times in 3.5 hours on the 2026-09-22 event, most likely the unpinned Alpine edge Chromium base in `wayland-kiosk/Dockerfile`. It produces roughly 200 coredump events a day, which rotates the host journal fast enough to threaten evidence for a real crash-forensics event; pin the Chromium version or try `--disable-gpu`/`--use-gl=swiftshader` and confirm the offset stops recurring. Not started; tracked here rather than in this integration because the fix belongs in ha_app_kiosk.
- 2026-09-22 crash_forensics.py's identifier resolution, the `boots/-1` line-count cap behaviour, and whether `hass.data[DATA_COMPONENT].send_command` is the only way to reach `/host/logs/...` text (versus a raw aiohttp session) are all unverified against a live Supervisor; see docs/CRASH-FORENSICS.md's Unverified section.
...
- 2026-09-11 SFTP logins into the audit chain: sshd logs "Accepted publickey for root from <ip> ... <fingerprint>" and refusals to the app log; a reader of that log (the Logs tab's Supervisor gateway, or the external audit ingest from inside the app) could turn them into `terminal_sftp_login` records. Not started.
...
- 2026-09-12 Terminal bundled font: the font picker offers only families installed on the viewing machine. Shipping JetBrains Mono (OFL) as a woff2 inside the panel bundle would make every browser render the same glyphs; about 100 KB per weight on a 700 KB bundle. Not done.
