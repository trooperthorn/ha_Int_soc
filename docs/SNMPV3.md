...
| snmpd drops to the `ha_soc_snmp` account after opening the listener (`agentuser` / `agentgroup`); its persistent directory must remain writable by that account so engine boots and time and localized USM keys survive a clean shutdown. The directory and the createUser file are root-owned with group `ha_soc_snmp` and mode 0770 / 0660: the daemon writes through the group, and the supervisor script can still replace both on a credential change without CAP_DAC_OVERRIDE. Releases through 2026.09.09.2 chowned the directory to `ha_soc_snmp` instead, which made every generation after the first fail with `Permission denied` on `persistent/snmpd.conf`; the script now reclaims ownership at startup. | Verified (code and Net-SNMP documentation) |
| Net-SNMP's persistent-store checkpoint (`read_config_store`) runs repeatedly while `snmpd` is already running as the dropped `ha_soc_snmp` account, not only once at startup while still root. Unix requires the executable bit on *every* path component to open a file beneath it, so `PERSIST_DIR`'s own 0770 grant is unreachable by that account unless its parent, `STATE_DIR` (`/data/ha_soc_snmp`), also grants the `ha_soc_snmp` group traversal. Releases through 2026.09.16 left `STATE_DIR` at mode 0700 owned `root:root`, so every checkpoint after the privilege drop failed at that traversal step and logged the misleading pair `Failed to create the persistent directory for /data/ha_soc_snmp/persistent/snmpd.conf` / `read_config_store open failure on /data/ha_soc_snmp/persistent/snmpd.conf` on a loop — the message names the full file path, not the directory that actually blocked access, and `PERSIST_DIR` and the file themselves were never the problem. Reproduced and fixed in a disposable Alpine/Net-SNMP 5.9.4 container with no AppArmor confinement applied, which also rules out the profile's capability set as a factor: the failure is pure DAC, present and absent purely as a function of `STATE_DIR`'s permission bits. The script now also chowns `STATE_DIR` to `root:ha_soc_snmp` and sets it 0710 (group execute-only, no read/list) so the dropped account can traverse into it without gaining any broader access. | Verified (reproduced and fixed in a throwaway container; Net-SNMP source, `read_config.c`) |
