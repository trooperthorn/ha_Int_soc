...
- Dashboard Files commands are namespaced `ha_soc/dashboards/*` and every `path` is relative to `<config>/dashboards`, posix-separated, `.yaml` or `.yml`, and never absolute. `list` returns `{enabled, root, root_exists, truncated, max_bytes, files: [{path, size, modified, too_large}]}`; `modified` is epoch seconds, `max_bytes` is 1048576, and `enabled: false` (with an empty `files`) is the shape returned when the owner has not set `dashboard_edit_enabled`. `read` returns `{path, content, sha256}`. `validate` takes `content` plus an optional `path` (the path only enables the `!include` target checks) and returns `{valid, errors, warnings}`, each diagnostic `{message, line, column}` with one-based line and column, or nulls when the parser reported no mark. `write` requires `path`, `content`, `expected_sha256` (64 lowercase hex, the digest from the read), and a `reason` of 1 to 500 characters, and returns `{path, sha256, previous_sha256, backup, warnings, bytes}` with `backup` relative to the configuration directory. Error codes: `dashboard_edit_disabled`, `not_allowed` (path refused), `not_found`, `too_large`, `invalid_yaml`, `conflict` (the file changed since the read), `write_failed`.
- Newly used Network routes, all read-only: `GET /info` (`applicationVersion`, the one route that is not site-scoped), `GET /sites/{id}/firewall/policies/ordering` (returns `orderedFirewallPolicyIds` as `{beforeSystemDefined, afterSystemDefined}`, two lists because user policies sit either side of the system-defined set), `GET /sites/{id}/acl-rules/ordering` (a flat `orderedAclRuleIds`), and `GET /sites/{id}/networks/{networkId}/references` (`referenceResources`, each `{resourceType, referenceCount, references?}`; `resourceType` is one of CLIENT, DEVICE, STATIC_ROUTE, OSPF_ROUTE, NEXT_AI, WIFI, NAT_RULE, SD_WAN, and the id list is present only for types the API models, so a count with no ids is normal). In the overview, `acl.ordering` and `firewall_policies.ordering` are null when the route did not answer, which is not the same as an empty ordering.
- `ha_soc/unifi_ledger/get` returns `{available, error, application_version, baseline, current, drift, history, findings}`. `current` and `baseline.snapshot` are snapshots: `{taken_at, application_version, site_id, digest, section_digests, sections, ordering}` where sections are `networks`, `zones`, `firewall_policies`, `acl_rules`, `devices`, each a list of rows with a stable `id` (a resource id, or a lowercase MAC for devices). `taken_at` is outside the digest. `drift` is `{total, sections: {added, removed, changed, ordering_changed, count}, baseline_digest, current_digest}`; `changed` rows carry `{id, name, changes: [{field, from, to}]}`. `ha_soc/unifi_ledger/accept` takes the 64-hex `digest` of the snapshot the panel displayed and errors `stale_snapshot` when the controller no longer matches it, or `not_available` when it could not be read completely.
...
- `known_rules` entries carry `family` "4" or "6" per table; `action` is `allow` for ACCEPT, `reject` for REJECT, and `deny` otherwise. Each entry is `{action, proto, ports, icmp_code, source, destination, interface, log, comment, family}`: `ports` is the `--dport` or `--dports` value, `icmp_code` the numeric `--icmp-type` or `--icmpv6-type` (`"any"` when absent), `proto` `icmp` for both `icmp` and `ipv6-icmp`, and `log` true when a `-j LOG` line with the same match preceded the action line. Core maps `icmp_code` to a type name and the legacy integer `port` to `ports` in `normalize_known_rule`.
- Rules delivered on `apply` carry Core's normalized shape plus `icmp_codes` (`{"4": code|"any"|null, "6": ...}`) for icmp rules; the add-on skips a family whose code is null.
- `firewall_capabilities`: `{multiport, comment, log, limit, reject, icmp}` booleans from `probe_capabilities`, sent on every resolution report and on the status heartbeat (`report_status`, every `STATUS_REPORT_EVERY` polls). Additional reject reasons: `rule N has an invalid icmp type`, `... invalid destination for family F`, `... invalid interface`, `... invalid comment`, `... invalid log flag`, and `rule N needs <extension>, which this host lacks`.
- The scanner's report carries `interfaces`: every host interface name except `lo`, from `ip -o link show`, with the `@parent` suffix stripped.
