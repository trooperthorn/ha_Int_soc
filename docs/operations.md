...
| `syslog_receiver_enabled`, `syslog_receiver_port` | False, 5514 | Opposite direction of the exporter above: an optional UDP listener in the Probe (`ha_soc_probe_syslog_receiver`) that ingests forwarded logs, compatible with the "logspout" HA add-on. Distinct default port from the exporter's 514 and SNMP's 161 to avoid an owner accidentally pointing logspout at the wrong service. UDP only this phase (see `docs/security.md`); no credentials. Entries are bounded ring-buffer storage (`SYSLOG_RECEIVER_MAX_ENTRIES`, oldest dropped first), fetched via the owner-gated `ha_soc/syslog_receiver/entries` command and shown in Logs → "Forwarded container logs (logspout)". |
...
- `DEFAULT_HYGIENE_SCAN_YAML_DASHBOARDS` False: the unused-resource check reads YAML-mode dashboard files only when the owner turns this on in Settings; only card `type` values are read and nothing is written.
...
- Unused-install checks (all informational): `integrations_without_entry` (custom integrations on disk that no entry, YAML block, or loaded dependency brings in), `entries_without_entities_or_devices` (loaded entries owning nothing in either registry; system, hardware, service, and virtual types excluded), `hacs_not_loaded` (HACS downloads with no set-up domain or no resource entry), `unused_dashboard_resources` (resources whose registered custom elements no dashboard references; resources whose bundle cannot be read statically are counted in the summary, never as unused). The last check evaluates only when every dashboard could be read, so with a YAML-mode dashboard present it stays silent until the `hygiene_scan_yaml_dashboards` setting is on.
...
UniFi suggestion write-back: `unifi_network_write_enabled` (default off) plus the `unifi_network_write_api_key` secret. Enabling it without the controller host and the write key is refused. With it on, the Suggested changes tab under Firewall Policies offers Apply on suggestions that name one policy or ACL rule; Apply disables that object on the controller and records `applied` with the read-back result. Failure reasons: `write_disabled`, `finding_not_current` (the suggestion no longer fires on a fresh snapshot), `no_automatic_remediation`, or the controller's own error text. Plan and Ignore work without write-back and only change HA SOC's store.
...
Rule fields: action allow, deny (DROP), or reject (REJECT); protocol tcp, udp, or icmp; a ports spec (`443`, `8000:8100`, `80,443`, at most fifteen entries) for tcp and udp or an ICMP type name for icmp; optional source and destination address or CIDR (both must be the same family); optional interface name (from the Probe's reported list, or typed); a log flag (kernel log, five per minute, prefix `HA_SOC:<comment>:`); an optional single-token comment shown in `iptables -S`. The card lists which optional extensions the host lacks (multiport, comment, log, limit, reject, icmp) and disables the matching options; a proposal that still needs one is refused with `unsupported_capabilities: <names>`. The Probe reports `firewall_capabilities` and `interfaces` on its status heartbeat (about every five minutes) and after every apply, so a fresh install shows all options enabled until the first report arrives.
