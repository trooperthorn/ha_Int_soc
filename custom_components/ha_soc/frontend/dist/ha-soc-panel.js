...
`,Ht={order:[],hidden:[]};function Bt(t,e){const s=new Map(t.map(t=>[t.id,t])),i=new Set,a=[];for(const t of e.order){const e=s.get(t);e&&!i.has(t)&&(a.push(e),i.add(t))}for(const e of t)i.has(e.id)||(a.push(e),i.add(e.id));return a}let Vt=class extends nt{constructor(){super(...arguments),this.sections=[],this.layout=Ht,this._dragId=null}render(){const t=Bt(this.sections,this.layout),e=new Set(this.layout.hidden);return V`
  ...
  `}t([ht({attribute:!1})],jt.prototype,"hass",void 0),t([ht({type:Boolean})],jt.prototype,"customizeMode",void 0),t([ut()],jt.prototype,"_layout",void 0);let Kt=class extends jt{constructor(){super(...arguments),this._users=[],this._risk={},this._loading=!0,this._error=null,this._busyUserId=null,this._sort=null,this._pwUserId=null,this._pwValue="",this._pwKeepSessions=!1,this._pwError=null,this._pwNotice=null,this._isOwner=!1}get viewId(){return"users"}connectedCallback(){super.connectedCallback(),this._load()}async _load(){this._loading=!0,this._error=null;try{const[t,e,s]=await Promise.all([ft(this.hass),$t(this.hass),Rt(this.hass).catch(()=>({is_owner:!1}))]);this._users=t,this._risk=e,this._isOwner=!!s.is_owner}catch(t){this._error=t?.message??String(t)}finally{this._loading=!1}}_adminTargetLocked(t){return!this._isOwner&&(t.is_owner||t.groups.includes("system-admin"))}_fmtDate(t){if(!t)return"never";return new Date(t).toLocaleString()}async _onDeactivate(t){if(confirm("Deactivate this user? All their sessions will be revoked.")){this._busyUserId=t;try{await((t,e)=>bt(t,{type:"ha_soc/users/deactivate",user_id:e}))(this.hass,t),await this._load()}finally{this._busyUserId=null}}}async _onRevokeAll(t){if(confirm("Revoke every interactive session for this user? Long-lived tokens are kept.")){this._busyUserId=t;try{await((t,e)=>bt(t,{type:"ha_soc/users/revoke_all_sessions",user_id:e}))(this.hass,t),await this._load()}finally{this._busyUserId=null}}}_onToggleResetPanel(t){this._pwUserId=this._pwUserId===t?null:t,this._pwValue="",this._pwKeepSessions=!1,this._pwError=null,this._pwNotice=null}async _onSubmitPassword(t){if(this._pwValue){this._busyUserId=t,this._pwError=null,this._pwNotice=null;try{const e=await((t,e,s,i)=>bt(t,{type:"ha_soc/users/set_password",user_id:e,password:s,revoke_sessions:i}))(this.hass,t,this._pwValue,!this._pwKeepSessions);this._pwNotice=e.sessions_revoked>0?`Password set. ${e.sessions_revoked} interactive session${1===e.sessions_revoked?"":"s"} revoked; long-lived tokens were kept.`:this._pwKeepSessions?"Password set. Existing sessions were kept at your request.":"Password set. No interactive sessions were active.",this._pwUserId=null,this._pwValue="",this._pwKeepSessions=!1}catch(t){this._pwError=t?.message??"Could not set the password."}finally{this._busyUserId=null}}}_renderPasswordPanel(t){return V`
                        ...
                        <table>
                          ...
                          <tbody>
                            ${c.map(t=>{const e=we[t.issue_category];return V`
                                <tr
                                  class="clickable"
                                  title="${t.title} — ${$e[t.issue_category]}. Open in Home Assistant's Devices page"
    ...
    `:V`<div class="card"><h3>Device SSH</h3><p class="muted">Loading…</p></div>`:V`
      ...
      `}_renderApLogAnalysis(t){return t.wireless_activity?t.clients.length?V`
        ...
        <tbody>
          ${t.clients.map(t=>V`
              <tr>
                ...
                <td>
                  ${t.attempts}
                  ${t.failures?V`<span class="muted">(${t.failures} failed)</span>`:W}
                </td>
                <td>
                  ${t.failures&&!t.ever_succeeded?V`<span class="pill pill-fail">${t.last_stage}</span>`:V`<span class="muted">${t.last_outcome}</span>`}
                </td>
                <td>
                  ${null!==t.worst_rssi?`${t.worst_rssi} dBm`:"—"}
                </td>
                <td>${t.last_reason??t.last_reason_code??"—"}</td>
              </tr>
            `)}
        </tbody>
      </table>
    `:V`<p class="muted" style="margin:6px 0;font-size:12px;">
        ...
        <div class="alert" style="margin:6px 0;">
          ...
          ${t.controller_unreachable?V`It is also full of failed controller informs
                (${t.inform_failures} in this excerpt), which is what an access point that
                cannot reach the controller produces.`:W}
        </div>
      `}_renderStoredIpsPosture(){const t=this._overview?.ips_posture;return t?V`<p class="muted" style="font-size:12px;">
      Threat Management posture last read from <span class="mono">${t.host}</span> at
      <span class="mono">${t.collected_at.replace("T"," ").slice(0,19)}</span>:
      ${t.posture.mode}${t.posture.exempt_networks.length?`, ${t.posture.exempt_networks.length} network(s) never alerted on`:""}. Run ips_config again after changing the gateway.
      ...
      </p>`}_renderAnalysis(t){return"ips_config"===t.kind?this._renderIpsPosture(t):"ips_block_log"===t.kind?this._renderIpsBlockLog(t):this._renderApLogAnalysis(t)}_renderIpsPosture(t){const e=Object.entries(t.parsed).filter(([,t])=>!t).map(([t])=>t),s="prevent"===t.mode?"good":"detect"===t.mode?"medium":"off"===t.mode?"high":"info",i=t=>null===t?"unknown":t?"yes":"no";return V`
      ${e.length?V`<div class="alert" style="margin:6px 0;">
            ${e.length} of 6 files did not come back (${e.join(", ")}). The lists
      ...
      <table style="margin:6px 0;">
        <tbody>
          <tr>
            <th>Mode</th>
            <td>
              <span class="pill ${s}"><span class="dot"></span>${t.mode}</span>
              ${"prevent"===t.mode?V`<span class="muted"> ${t.drop_categories.length} categories block${null!==t.block_time_seconds?`, ${t.block_time_seconds} s per block`:""}</span>`:"detect"===t.mode?V`<span class="muted"> ${t.alert_categories.length} categories alert, none block</span>`:W}
            </td>
          </tr>
          <tr>
            <th>Threat logging</th>
            <td>${i(t.logging_threat_event)}</td>
          </tr>
          <tr>
            <th>SSL inspection</th>
            <td>${i(t.ssl_inspection)}${null!==t.suricata_version?V`<span class="muted"> · Suricata ${t.suricata_version}</span>`:W}</td>
          </tr>
          <tr>
            <th>Never alerted on</th>
            <td>
              ${t.parsed.reputation||t.parsed.threshold?t.exempt_networks.length||t.suppressed_networks.length?V`<span class="mono">${[...new Set([...t.exempt_networks,...t.suppressed_networks])].join(", ")}</span>
                      <span class="muted"> (allowlisted in both directions for every signature)</span>`:"none":"unknown"}
            </td>
          </tr>
          <tr>
            <th>HOME_NET</th>
            <td>${t.parsed.homenet?V`<span class="mono">${t.home_networks.join(", ")}</span>`:"unknown"}</td>
          </tr>
          <tr>
            <th>Inspected interfaces</th>
            <td>${t.parsed.interfaces?V`<span class="mono">${t.interfaces.map(t=>t.interface).join(", ")}</span>`:"unknown"}</td>
          </tr>
        </tbody>
      </table>
    `}_renderIpsBlockLog(t){if(!t.rows.length)return V`<p class="muted" style="margin:6px 0;font-size:12px;">
        No threat or firewall-policy blocks in the controller's alert collection${t.unparsed?` (${t.unparsed} line(s) could not be read)`:""}.
      </p>`;const e=t=>t.name?V`${t.name}<span class="muted"> ${t.address??t.mac??""}</span>`:V`<span class="mono">${t.address??t.mac??"—"}</span>`;return V`
      <p class="muted" style="margin:6px 0;font-size:12px;">
        Last 24 h: ${t.threat_blocks_24h} threat block(s), ${t.firewall_blocks_24h} policy block(s),
        ${t.sources_24h.length} distinct source(s). Showing the newest ${t.rows.length}.
        ${t.unparsed?`${t.unparsed} line(s) could not be read.`:""}
      </p>
      <div class="table-wrap">
        <table style="margin:6px 0;">
          <thead>
          ...
          <tbody>
            ${t.rows.map(t=>V`
                <tr>
                  <td class="mono">${t.time?t.time.replace("T"," ").slice(0,19):"—"}</td>
                  <td>${"threat"===t.kind?"Threat":"firewall"===t.kind?"Policy":t.key}</td>
                  <td>${t.severity?V`<span class="pill ${"VERY_HIGH"===t.severity||"HIGH"===t.severity?"high":"medium"}"><span class="dot"></span>${t.severity.toLowerCase().replace("_"," ")}</span>`:"—"}</td>
                  <td>${e(t.source)}</td>
    ...
    `}_renderSshRun(t){return V`
      ...
      ${t.results.map(t=>V`
          <div style="margin-bottom:10px;">
            ...
            ${t.analysis?this._renderAnalysis(t.analysis):W}
