...
`;let Ct=class extends rt{constructor(){super(...arguments),this._users=[],this._risk={},this._loading=!0,this._busyUserId=null}connectedCallback(){super.connectedCallback(),this._load()}async _load(){this._loading=!0;try{const[t,e]=await Promise.all([gt(this.hass),_t(this.hass)]);this._users=t,this._risk=e}finally{this._loading=!1}}_fmtDate(t){if(!t)return"never";return new Date(t).toLocaleString()}async _onDeactivate(t){if(confirm("Deactivate this user? All their sessions will be revoked.")){this._busyUserId=t;try{await((t,e)=>vt(t,{type:"ha_soc/users/deactivate",user_id:e}))(this.hass,t),await this._load()}finally{this._busyUserId=null}}}async _onRevokeAll(t){if(confirm("Revoke every interactive session for this user? Long-lived tokens are kept.")){this._busyUserId=t;try{await((t,e)=>vt(t,{type:"ha_soc/users/revoke_all_sessions",user_id:e}))(this.hass,t),await this._load()}finally{this._busyUserId=null}}}async _onResetPassword(t){const e=prompt("New password for this user (owner-only action):");if(e){this._busyUserId=t;try{const s=await((t,e,s)=>vt(t,{type:"ha_soc/users/set_password",user_id:e,password:s}))(this.hass,t,e);s&&!1===s.ok&&alert("Could not set password — only the account owner can reset another user's password.")}finally{this._busyUserId=null}}}render(){return this._loading?j`<div class="empty">Loading users…</div>`:this._users.length?j`
      ...
      <div class="card">
        ...
        <div class="toolbar">
          <select @change=${this._onCategoryChange}>
            ${Et.map(t=>j`<option value=${t} ?selected=${t===this._category}>${t||"All categories"}</option>`)}
          </select>
                    ...
                    <tbody>
                      ${p.map(t=>j`
                          <tr
                            ...
                            @click=${()=>Ut(`/config/devices/device/${t.device_id}`)}
                          >
                ...
                <div class="issues-list">
                  ${s.integrations.map(t=>j`
                      <div
                        ...
                        @click=${()=>Ut(`/config/devices/dashboard?historyBack=1&config_entry=${t.entry_id}`)}
                      >
    ...
    `:q}};function Ft(t){const e=t.match(/^homeassistant\.components\.([^.]+)/);if(e)return e[1];const s=t.match(/^custom_components\.([^.]+)/);return s?s[1]:t.split(".")[0]}Ht.styles=[St,n`
      h2.section-title {
        font-size: 13px;
        text-transform: uppercase;
...
${t.exception}</pre
      ...
      <div class="card">
        ...
        ${this._applyResult?j`
              <div class="card" style="margin-top:12px;background:rgba(67,160,71,0.08);">
                <strong>Applied.</strong> ${Object.entries(this._applyResult.fixed).filter(([,t])=>t>0).map(([t,e])=>`${e} ${Vt[t]??t}`).join(", ")||"Nothing needed changing."}
    ...
    `}};jt.styles=St,t([pt({attribute:!1})],jt.prototype,"hass",void 0),t([ut()],jt.prototype,"_entities",void 0),t([ut()],jt.prototype,"_oldEntityId",void 0),t([ut()],jt.prototype,"_newEntityId",void 0),t([ut()],jt.prototype,"_report",void 0),t([ut()],jt.prototype,"_finding",void 0),t([ut()],jt.prototype,"_applying",void 0),t([ut()],jt.prototype,"_applyResult",void 0),t([ut()],jt.prototype,"_broken",void 0),t([ut()],jt.prototype,"_brokenLoading",void 0),jt=t([dt("ha-soc-entity-remap-view")],jt);const Bt=1048576;let qt=class extends rt{constructor(){super(...arguments),this._settings=null,this._loading=!0}connectedCallback(){super.connectedCallback(),this._load()}async _load(){this._loading=!0;try{this._settings=await(t=this.hass,vt(t,{type:"ha_soc/settings/get"}))}finally{this._loading=!1}var t}async _update(t,e){if(!this._settings)return;const s=this._settings;this._settings={...this._settings,[t]:e};try{this._settings=await(i=this.hass,a={[t]:e},vt(i,{type:"ha_soc/settings/set",...a}))}catch(t){throw this._settings=s,t}var i,a}render(){if(this._loading||!this._settings)return j`<div class="empty">Loading settings…</div>`;const t=this._settings;return j`
      <div class="card">
      ...
      <div class="card">
        ...
        <label class="settings-row">
          ...
          <input
            type="number"
            min="1"
            .value=${String(Math.round(t.audit_max_bytes/Bt))}
            @change=${t=>this._update("audit_max_bytes",Math.round(Number(t.target.value)*Bt))}
          />
        </label>
      </div>
    ...
    `}};qt.styles=St,t([pt({attribute:!1})],qt.prototype,"hass",void 0),t([ut()],qt.prototype,"_settings",void 0),t([ut()],qt.prototype,"_loading",void 0),qt=t([dt("ha-soc-settings-view")],qt);const Kt=[{id:"dashboard",label:"Dashboard"},{id:"users",label:"Users & Access"},{id:"audit",label:"Audit Log"},{id:"permissions",label:"Permissions"},{id:"scanner",label:"Scanner"},{id:"logs",label:"Logs"},{id:"peripherals",label:"Local Peripherals"},{id:"entity_remap",label:"Entity ReMap"},{id:"settings",label:"Settings"}];let Wt=class extends rt{constructor(){super(...arguments),this._tab="dashboard",this._access=null}connectedCallback(){super.connectedCallback(),this._loadAccess()}async _loadAccess(){try{this._access=await(t=this.hass,vt(t,{type:"ha_soc/access/info"}))}catch{this._access={is_owner:!1,access_level:"owner_only",allowed:!1}}var t}render(){return null===this._access?j`<div class="header">🛡️ HA SOC</div>`:this._access.allowed?j`
    ...
    `:j`
      ...
      `}_onNavigate(t){this._tab=t.detail.tab}_renderTab(){switch(this._tab){case"users":return j`<ha-soc-users-view .hass=${this.hass}></ha-soc-users-view>`;case"audit":return j`<ha-soc-audit-view .hass=${this.hass}></ha-soc-audit-view>`;case"permissions":return j`<ha-soc-permissions-view .hass=${this.hass}></ha-soc-permissions-view>`;case"scanner":return j`<ha-soc-scanner-view .hass=${this.hass}></ha-soc-scanner-view>`;case"logs":return j`<ha-soc-logs-view .hass=${this.hass}></ha-soc-logs-view>`;case"peripherals":return j`<ha-soc-peripherals-view .hass=${this.hass}></ha-soc-peripherals-view>`;case"entity_remap":return j`<ha-soc-entity-remap-view .hass=${this.hass}></ha-soc-entity-remap-view>`;case"settings":return j`<ha-soc-settings-view .hass=${this.hass}></ha-soc-settings-view>`;default:return j`<ha-soc-dashboard-view .hass=${this.hass}></ha-soc-dashboard-view>`}}};Wt.styles=n`
    :host {
      display: block;
  ...
  `,t([pt({attribute:!1})],Wt.prototype,"hass",void 0),t([pt({attribute:!1})],Wt.prototype,"narrow",void 0),t([pt({attribute:!1})],Wt.prototype,"panel",void 0),t([ut()],Wt.prototype,"_tab",void 0),t([ut()],Wt.prototype,"_access",void 0),Wt=t([dt("ha-soc-panel")],Wt);export{Wt as HaSocPanel};
