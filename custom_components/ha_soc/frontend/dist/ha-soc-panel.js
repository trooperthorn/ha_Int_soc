 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function ut(t){return pt({...t,state:!0,attribute:!1})}const gt=(t,e)=>t.callWS(e),vt=t=>gt(t,{type:"ha_soc/users/list"}).then(t=>t.users),_t=t=>gt(t,{type:"ha_soc/risk/list"}).then(t=>t.risk),yt=(t,e)=>gt(t,{type:"ha_soc/detections/list",status:e}).then(t=>t.detections),bt=(t,e,s)=>gt(t,{type:"ha_soc/detections/set_status",detection_id:e,status:s}),mt=t=>gt(t,{type:"ha_soc/vulns/list"}).then(t=>t.findings),ft=t=>gt(t,{type:"ha_soc/logs/fault"}),wt=t=>gt(t,{type:"ha_soc/health/list"}),$t=t=>gt(t,{type:"ha_soc/dashboard/devices"}),xt=t=>gt(t,{type:"ha_soc/dashboard/integrations"}),kt=t=>gt(t,{type:"ha_soc/probe/status"}),St=t=>gt(t,{type:"ha_soc/firewall/status"}),At=t=>gt(t,{type:"ha_soc/peripherals/list"}),Ct=t=>gt(t,{type:"ha_soc/entity_remap/broken_references"}).then(t=>t.broken),Pt=t=>gt(t,{type:"ha_soc/security_health/list"}),Et=n`
  :host {
    display: block;
    padding: 16px;
    max-width: 1400px;
...
`;let zt=class extends rt{constructor(){super(...arguments),this._users=[],this._risk={},this._loading=!0,this._busyUserId=null}connectedCallback(){super.connectedCallback(),this._load()}async _load(){this._loading=!0;try{const[t,e]=await Promise.all([vt(this.hass),_t(this.hass)]);this._users=t,this._risk=e}finally{this._loading=!1}}_fmtDate(t){if(!t)return"never";return new Date(t).toLocaleString()}async _onDeactivate(t){if(confirm("Deactivate this user? All their sessions will be revoked.")){this._busyUserId=t;try{await((t,e)=>gt(t,{type:"ha_soc/users/deactivate",user_id:e}))(this.hass,t),await this._load()}finally{this._busyUserId=null}}}async _onRevokeAll(t){if(confirm("Revoke every interactive session for this user? Long-lived tokens are kept.")){this._busyUserId=t;try{await((t,e)=>gt(t,{type:"ha_soc/users/revoke_all_sessions",user_id:e}))(this.hass,t),await this._load()}finally{this._busyUserId=null}}}async _onResetPassword(t){const e=prompt("New password for this user (owner-only action):");if(e){this._busyUserId=t;try{const s=await((t,e,s)=>gt(t,{type:"ha_soc/users/set_password",user_id:e,password:s}))(this.hass,t,e);s&&!1===s.ok&&alert("Could not set password — only the account owner can reset another user's password.")}finally{this._busyUserId=null}}}render(){return this._loading?B`<div class="empty">Loading users…</div>`:this._users.length?B`
          ...
          <tbody>
            ${this._users.map(t=>{const e=this._risk[t.id];return B`
                <tr class=${t.is_active?"":"row-disabled"}>
                  <td>
                    <div>${t.name??t.id}</div>
                    ${t.is_owner?B`<span class="tag enforced">owner</span>`:G}
                    ${t.is_active?G:B`<span class="tag cosmetic">deactivated</span>`}
                  </td>
                  ...
                  <td>
                    ...
                    ${t.last_login_ip?B`<div class="muted">${t.last_login_ip}</div>`:G}
                  </td>
                  <td>
      ...
      <div class="card">
        ...
        <div class="toolbar">
          <select @change=${this._onCategoryChange}>
            ${Rt.map(t=>B`<option value=${t} ?selected=${t===this._category}>${t||"All categories"}</option>`)}
          </select>
    ...
    `}_renderProbeCard(){const t=this._probe;if(!t)return G;if(!t.supervisor)return B`
        <div class="card">
      ...
      <div class="card">
        <h3>
          ...
          ${t.update_available?B`<span class="tag cosmetic">update available</span>`:G}
        </h3>
        <p class="muted" style="margin-top:-8px;font-size:12.5px;">
    ...
    `}_renderFirewallCard(){const t=this._probe,e=this._firewall;return t?.supervisor&&t?.installed&&e?B`
      <div class="card">
        ...
        ${this._fwError?B`<p style="color:var(--error-color,#db4437);font-size:12.5px;margin-top:10px;">${this._fwError}</p>`:G}
      </div>
    `:G}_renderFirewallPending(t){const e=Math.max(0,Math.round((new Date(t.expires_at).getTime()-Date.now())/1e3)),s={testing:t.applied_at?"Testing — live on the host":"Queued — waiting for the add-on to apply",confirmed:"Confirmed — waiting for the add-on to acknowledge",reverted:"Reverting — waiting for the add-on to acknowledge",expired:"Window expired — reverting automatically"};return B`
      ...
      <div class="toolbar" style="margin-top:12px;">
        ...
        >
          Apply${"testing"===t.status?B` (${e}s to auto-revert)`:G}
        </button>
        <button
          class="ha-btn danger"
          ?disabled=${this._fwSubmitting||"testing"!==t.status}
        ...
        <div class="card" id="devices-card">
          ...
          ${this._deviceStatusFilter?B`
                <div class="filter-chip" @click=${()=>this._deviceStatusFilter=null}>
                  ${Vt.find(t=>t.key===this._deviceStatusFilter)?.label} ✕
                </div>
              `:G}
    ...
    `}_sortArrow(t){return this._deviceSort.key!==t?G:B`<span class="arrow">${"asc"===this._deviceSort.dir?"▲":"▼"}</span>`}_renderSecurityCard(){const t=this._security;if(!t)return G;const e={};for(const s of t.entities)(e[s.domain]??=[]).push(s);return B`
      <div class="card">
        <h3>
        ...
        <div class="security-health-grid">
          ${Object.entries(Mt).filter(([e])=>t.sources_enabled[e]??!0).map(([t,s])=>{const i=e[t]??[],a=i.filter(t=>t.problem).length,o=i.filter(t=>t.low_battery).length;return B`
                <div
                  class="security-source-tile ${i.length?"clickable":""}"
                  title=${i.length?`View ${s.toLowerCase()} in Home Assistant's Devices page`:""}
                  @click=${()=>i.length&&Ot(function(t){return`/config/devices/dashboard?historyBack=1&domain=${t}`}(t))}
                >
    ...
    `}_filter(t,e){const s=e.trim().toLowerCase();return s?t.filter(t=>[t.name,t.ipv4,t.ipv6,t.mac,t.ssid,t.integration_match?.domain].filter(Boolean).some(t=>String(t).toLowerCase().includes(s))):t}_paginate(t,e,s){return"all"===s?t:t.slice(e*s,e*s+s)}render(){if(this._loading)return B`<div class="empty">Loading network…</div>`;if(this._error)return B`<div class="alert">Could not load the Network overview: ${this._error}</div>`;const t=this._overview;return t?t.configured?t.reachable?B`
      ...
      ${this._renderClientsTable(t)} ${this._renderDevicesTable(t)} ${this._renderProtectCard(t)}
      <div class="footer">
    ...
    `:B`
      ...
      `:B`
        <div class="card">
          ...
          <button class="ha-btn" @click=${()=>Dt(this,"settings")}>
            Open Settings
          </button>
        </div>
        ${this._renderProtectCard(t)}
    ...
    `:G}_renderStats(t){const e="online"===t.status,s=t.internet_connected;return B`
      <div class="stat-row">
        <div class="stat-tile">
    ...
    `}_renderSsid(t){if(!t.clients_per_ssid.length)return G;const e=Math.max(...t.clients_per_ssid.map(t=>t.count),1);return B`
      <div class="card">
    ...
    `}_renderRow(t,e={}){const s=t;return B`
      <tr>
        <td>
          ...
          ${e.model||t.wired?e.model&&s.state?B`<div class="muted" style="font-size:11px;">${s.state.toLowerCase()}</div>`:G:B`<div class="muted" style="font-size:11px;">wireless</div>`}
        </td>
    ...
    `}_renderClientsTable(t){const e=this._filter(t.clients,this._clientSearch),s=this._paginate(e,this._clientPage,this._clientPageSize);return B`
      <div class="card">
        <h3>Clients (${e.length})</h3>
        <div class="toolbar">
          <input
            type="text"
        ...
        ${0===e.length?B`<div class="empty">No clients match.</div>`:B`
              ...
              ${this._renderPager(e.length,this._clientPage,this._clientPageSize,t=>this._clientPage=t,t=>{this._clientPageSize=t,this._clientPage=0})}
            `}
        <div class="note">
      ...
      <div class="card">
        ...
        ${0===e.length?B`<div class="empty">No network devices match.</div>`:B`
              <div class="table-wrap">
                <table>
                  <thead>
                    ${this._colHeaders({model:!0})}
                  </thead>
                  <tbody>
                    ${s.map(t=>this._renderRow(t,{model:!0}))}
                  </tbody>
                </table>
              </div>
    ...
    `}_renderProtectCard(t){const e=t.protect;return e.configured?B`
      <div class="card">
        <h3>UniFi Protect</h3>
        ${e.reachable?B`
              <div class="stat-row" style="margin-bottom:0;">
                <div class="stat-tile">
                  ...
                  <div class="value">
                    <span class="dot ${e.cameras_online===e.camera_count?"good":"bad"}"></span>${e.cameras_online} / ${e.camera_count}
                  </div>
                </div>
              </div>
            `:B`<div class="muted" style="font-size:13px;">
              Configured but not reachable${e.error?B` — ${e.error}`:""}.
            </div>`}
      </div>
    `:G}};function Yt(t){const e=t.match(/^homeassistant\.components\.([^.]+)/);if(e)return e[1];const s=t.match(/^custom_components\.([^.]+)/);return s?s[1]:t.split(".")[0]}Jt.styles=[Et,n`
      .stat-row {
        display: grid;
        grid-template-columns: repeat(auto-fit, minmax(150px, 1fr));
        gap: 12px;
    ...
    `:G}render(){const t=this._filtered;return B`
                ...
                <tbody>
                  ${t.map((t,e)=>{const s=this._expanded.has(e);return B`
                      ...
                      >
                        ...
                        <td>
                          <span class="log-level ${function(t){const e=t.toUpperCase();return Qt.includes(e)?e.toLowerCase():"info"}(t.level)}"
                            ><span class="dot"></span>${t.level}</span
                          >
                        </td>
                        <td class="muted">${Yt(t.name)}</td>
                        <td>
                          ${t.message[t.message.length-1]}
                          ${t.source?B`<div class="muted" style="font-size:11px;">${t.source[0]}:${t.source[1]}</div>`:G}
                        </td>
...
${t.exception}</pre
    ...
    `:G}render(){const t=this._report,e=!!t&&t.editable_count>0&&!!this._newEntityId&&this._newEntityId!==this._oldEntityId;return B`
      <div class="card">
        <h3>Entity ReMap</h3>
        <p class="muted" style="margin-top:-8px;font-size:12.5px;">
        ...
        ${this._applyResult?B`
              <div class="card" style="margin-top:12px;background:rgba(67,160,71,0.08);">
                <strong>Applied.</strong> ${Object.entries(this._applyResult.fixed).filter(([,t])=>t>0).map(([t,e])=>`${e} ${ee[t]??t}`).join(", ")||"Nothing needed changing."}
      ...
      <div class="card">
        <h3>
          Entities referenced but not found (${this._filteredBroken().length}${this._brokenFilter?B` of ${this._broken.length}`:G})
        </h3>
        <p class="muted" style="margin-top:-8px;font-size:12.5px;">
    ...
    `}};se.styles=Et,t([pt({attribute:!1})],se.prototype,"hass",void 0),t([ut()],se.prototype,"_entities",void 0),t([ut()],se.prototype,"_oldEntityId",void 0),t([ut()],se.prototype,"_newEntityId",void 0),t([ut()],se.prototype,"_report",void 0),t([ut()],se.prototype,"_finding",void 0),t([ut()],se.prototype,"_applying",void 0),t([ut()],se.prototype,"_applyResult",void 0),t([ut()],se.prototype,"_broken",void 0),t([ut()],se.prototype,"_brokenLoading",void 0),t([ut()],se.prototype,"_brokenFilter",void 0),se=t([dt("ha-soc-entity-remap-view")],se);const ie={core:"Core",hacs:"HACS",custom:"Custom"},ae={core:"good",hacs:"medium",custom:"high"},oe={custom_repo:"Custom repo",custom_source_list:"Custom source-list"};let ne=class extends rt{constructor(){super(...arguments),this._overview=null,this._loading=!0,this._refreshing=!1,this._search="",this._tierFilter="all",this._limit=25}connectedCallback(){super.connectedCallback(),this._load()}async _load(){this._loading=!0;try{this._overview=await(t=this.hass,gt(t,{type:"ha_soc/integration_security/list"}))}finally{this._loading=!1}var t}async _onRefresh(){this._refreshing=!0;try{await(t=this.hass,gt(t,{type:"ha_soc/integration_security/refresh"})),await this._load()}finally{this._refreshing=!1}var t}_filtered(){const t=this._overview?.integrations??[],e=this._search.trim().toLowerCase();return t.filter(t=>"all"===this._tierFilter||t.tier===this._tierFilter).filter(t=>!e||t.name.toLowerCase().includes(e)||t.domain.toLowerCase().includes(e)).sort((t,e)=>t.name.localeCompare(e.name))}render(){if(this._loading||!this._overview)return B`<div class="empty">Loading integrations…</div>`;const t=this._overview,e=this._filtered(),s=e.slice(0,this._limit);return B`
      <div class="card">
        ...
        ${t.github_configured?t.refreshed_at?B`<p class="muted" style="font-size:12px;margin:0 0 4px;">
              ...
              </p>`:G:B`<p class="muted" style="font-size:12px;margin:0 0 4px;">
    ...
    `}_notCollected(){return B`<span class="muted" title="No GitHub token, or no repo URL discovered">—</span>`}_renderRow(t){const e=t.github;return B`
      <tr>
        ...
        <td>
          <span class="pill ${ae[t.tier]}"><span class="dot"></span>${ie[t.tier]}</span>
        </td>
    ...
    `}};ne.styles=Et,t([pt({attribute:!1})],ne.prototype,"hass",void 0),t([ut()],ne.prototype,"_overview",void 0),t([ut()],ne.prototype,"_loading",void 0),t([ut()],ne.prototype,"_refreshing",void 0),t([ut()],ne.prototype,"_search",void 0),t([ut()],ne.prototype,"_tierFilter",void 0),t([ut()],ne.prototype,"_limit",void 0),ne=t([dt("ha-soc-integration-security-view")],ne);const re=1048576,le=[{domain:"lock",label:"Lock entities (any integration)"},{domain:"siren",label:"Siren entities (any integration)"},{domain:"valve",label:"Valve entities (any integration)"}],de=[{domain:"kidde_homesafe",label:"Kidde HomeSafe"},{domain:"elkm1",label:"Elk-M1 Security"},{domain:"unifiprotect",label:"UniFi Protect"},{domain:"keymaster",label:"Keymaster"},{domain:"emporia_vue",label:"Emporia Vue"}];let ce=class extends rt{constructor(){super(...arguments),this._settings=null,this._security=null,this._loading=!0}connectedCallback(){super.connectedCallback(),this._load()}async _load(){this._loading=!0;try{this._settings=await(t=this.hass,gt(t,{type:"ha_soc/settings/get"}));try{this._security=await Pt(this.hass)}catch{this._security=null}}finally{this._loading=!1}var t}async _update(t,e){if(!this._settings)return;const s=this._settings;this._settings={...this._settings,[t]:e};try{this._settings=await(i=this.hass,a={[t]:e},gt(i,{type:"ha_soc/settings/set",...a}))}catch(t){throw this._settings=s,t}var i,a}_updateSecuritySource(t,e){this._settings&&this._update("security_sources_enabled",{...this._settings.security_sources_enabled,[t]:e})}_renderSecretField(t,e,s){return B`
    ...
    `}render(){if(this._loading||!this._settings)return B`<div class="empty">Loading settings…</div>`;const t=this._settings;return B`
      ...
      <div class="card">
        ...
        ${de.map(({domain:t,label:e})=>this._renderIntegrationRow(t,e))}
      </div>
    ...
    `:B`
      ...
      `}_onNavigate(t){this._tab=t.detail.tab}_renderTab(){switch(this._tab){case"users":return B`<ha-soc-users-view .hass=${this.hass}></ha-soc-users-view>`;case"audit":return B`<ha-soc-audit-view .hass=${this.hass}></ha-soc-audit-view>`;case"permissions":return B`<ha-soc-permissions-view .hass=${this.hass}></ha-soc-permissions-view>`;case"scanner":return B`<ha-soc-scanner-view .hass=${this.hass}></ha-soc-scanner-view>`;case"logs":return B`<ha-soc-logs-view .hass=${this.hass}></ha-soc-logs-view>`;case"peripherals":return B`<ha-soc-peripherals-view .hass=${this.hass}></ha-soc-peripherals-view>`;case"network":return B`<ha-soc-network-view .hass=${this.hass}></ha-soc-network-view>`;case"entity_remap":return B`<ha-soc-entity-remap-view .hass=${this.hass}></ha-soc-entity-remap-view>`;case"integration_security":return B`<ha-soc-integration-security-view .hass=${this.hass}></ha-soc-integration-security-view>`;case"settings":return this._access?.is_owner?B`<ha-soc-settings-view .hass=${this.hass}></ha-soc-settings-view>`:B`<div class="denied"><div class="icon">🔒</div><h2>Owner only</h2>
            <p>The Settings tab is available to the account owner only.</p></div>`;default:return B`<ha-soc-dashboard-view .hass=${this.hass}></ha-soc-dashboard-view>`}}};pe.styles=n`
    :host {
      display: block;
  ...
  `,t([pt({attribute:!1})],pe.prototype,"hass",void 0),t([pt({attribute:!1})],pe.prototype,"narrow",void 0),t([pt({attribute:!1})],pe.prototype,"panel",void 0),t([ut()],pe.prototype,"_tab",void 0),t([ut()],pe.prototype,"_access",void 0),t([ut()],pe.prototype,"_version",void 0),t([ut()],pe.prototype,"_probe",void 0),pe=t([dt("ha-soc-panel")],pe);export{pe as HaSocPanel};
